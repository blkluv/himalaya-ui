...
The Input contain an additional Hooks, see subsection [useInput](/web5/use-input) for details.

<Playground
  desc="Basic input field."
  scope={{ Input }}
  code={`
<Input placeholder="Himalaya" />
`}
/>
...
<Playground
  ...
  scope={{ Input }}
  code={`
<Input placeholder="Himalaya" width="100%" />
`}
/>
...
<Playground
  ...
  code={`
<>
  <Input clearable initialValue="Himalaya" placeholder="Himalaya" />
</>
`}
/>
...
() => {
  ...
  return (
    <Input value={value} onChange={handler} placeholder="Himalaya" />
  )
}
`}
/>
...
<Attributes edit="/pages/components/input.mdx">
